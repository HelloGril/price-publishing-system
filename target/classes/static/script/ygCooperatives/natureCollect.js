...
$('#natureCollectGrid').datagrid({
    ...
    columns: [[
        ...
        {field:'plantingZoneId',title:'种植区',width:100,sortable:true, formatter : function(value, row, index) {
            return dictValue(plantingZoneDict, 'id', 'name', value)
        }}
    ]]
});
...
$('.plantingZoneId').combobox({
    url: base + '/comboBox/id/plantingZone',
    valueField: 'code',
    textField: 'name',
    onShowPanel:function () {
        $(this).combobox('reload')
...
$.ajax({
    type : "get",
    url : base+'/plantingZone/findIdAndName',
    cache : true,
    async : false,
    dataType : "json",
    success : function(obj) {
