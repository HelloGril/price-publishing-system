...
$('#cooperativesGrid').datagrid({
    ...
    columns: [[
        ...
        {field: 'maxAirTem', title: '最高空气温度', align: 'center', sortable: true},
        {field: 'minAirTem', title: '最低空气温度', align: 'center', sortable: true},
        {field: 'maxSoilTem', title: '最高土壤温度', align: 'center', sortable: true},
        {field: 'minSoilTem', title: '最低土壤温度', align: 'center', sortable: true},
        {field: 'standardSoilHumidity', title: '标准土壤湿度', align: 'center', sortable: true},
        {field: 'standardAirHumidity', title: '标准空气湿度', align: 'center', sortable: true},
...
function openUpdateCooperativeWin() {
    ...
    if (row) {
        ...
        honorValue= $("#honor").combobox('getValue');
        $("#honor").combobox('setValue','');
    } else {
        pop('提示', "请选择要修改的数据！");
    }
}
...
function updateCooperatives() {
    if ($('#updateCooperativesForm').form('validate')) {
    	var honor = $("#honor").combobox('getValues');
    	var newHonr=honor[0];
    	for(var i =1;i<honor.length;i++){
    		newHonr=newHonr+','+honor[i];
    	}
    	if(newHonr==''){
    		newHonr=honorValue;
    	}
        $.ajax({
            ...
            data: 'honor='+newHonr+'&'+$('#updateCooperativesForm').serialize(),
            dataType: 'json',
            success: function (obj) {
                if (obj.success) {
                    $('#cooperativesUpdateWin').window('close');
...
function saveCooperatives() {
    if ($('#saveCooperativesForm').form('validate')) {
        ...
        $.ajax({
            ...
            data: 'honor='+newHonr+'&'+$('#saveCooperativesForm').serialize(),
            dataType: 'json',
            success: function (obj) {
                if (obj.success) {
